locals {
  ...
  target_loop = flatten([for i in local.target : [for obj, val in i : {
    ...
    sat_fallback_type_trans           = contains(keys(val.translated_packet.fallback), "translated_addresses") ? "translated-address" : null
    sat_fallback_type_inter           = contains(keys(val.translated_packet.fallback), "interface_address") ? "interface-address" : null
        ...
        source {
          ...
          dynamic "static_ip" {
            ...
            content {
              ...
              bi_directional     = lookup(rule.value.translated_packet.static_ip, "bi_directional", false)
            }
          }
        }
      }
